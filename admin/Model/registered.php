...
function get_item(){
    ...
    $item  =  $db->row("select * from #_registered where id=:id");
    if(!$item) $func->transfer("Dữ liệu không có thực", $_SESSION['links_re']);	

    $db->bindMore(array("id"=>$id));
    $db->query("UPDATE table_registered SET view =1 WHERE  id = :id");
}
...
function delete_item(){
		...
		for ($i=0 ; $i<count($listid) ; $i++){
			$id=$listid[$i]; 
			$db->bindMore(array("id"=>$id));
			$row = $db->row("select * from #_registered where id=:id ");
			if($row){
				$db->bindMore(array("id"=>$id));
				$db->query("delete from #_registered where id=:id ");
			}
		}
		$func->redirect($_SESSION['links_re']);
	}
