...
<div class="control_frm">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.html?com=cate&act=add_item<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span>Thêm <?=$title_main?></span></a></li>
...
<form name="supplier" id="validate" class="form" action="index.html?com=cate&act=save_item<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
	<div class="widget">
		...
		<div class="formRow">
			...
			<div class="formRight">
			<div class="mt10"><img src="<?=_upload_cate.$item['thumb']?>"  alt="NO PHOTO" width="100" /></div>
			</div>
			<div class="clear"></div>
		</div>
		<?php } } ?>
        ...
        foreach ($config['lang'] as $key => $value) {?>
        <div class="formRow lang_hidden lang_<?=$key?> <?php if($config['activelang']==$key){ echo "active";}?>">
            <label>Tiêu đề 2 (<?=$value?>)</label>
            <div class="formRight">
                <input type="text" name="shortname_<?=$key?>" title="Nhập tên ngắn (<?=$value?>)" id="shortname_<?=$key?>" class="tipS validate[required]" value="<?=@$item['shortname_'.$key]?>" />
            </div>
            <div class="clear"></div>
        </div>
        <?php } } ?>
	...
	<div class="widget">
		...
		<div class="formRow">
			<div class="formRight">
            	...
            	<a href="index.html?com=cate&act=man_item<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" onClick="if(!confirm('Bạn có muốn thoát không ? ')) return false;" title="" class="button tipS" original-title="Thoát">Thoát</a>
			</div>
			<div class="clear"></div>
		</div>
