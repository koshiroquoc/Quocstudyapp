...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
            <li><a href="index.html?com=post&act=man<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span>Quản lý <?=$title_main ?></span></a></li>
            <?php if($_GET['keyword']!=''){ ?>
...
<form name="f" id="f" method="post">
    <?php if($type!='albumxxx'){?>
        <div class="control_frm" style="margin-top:0;">
            <div style="float:left;">
                <input type="button" class="blueB" value="Thêm" onclick="location.href='index.html?com=post&act=add<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>'" />
    ...
    <tbody>
     <?php for($i=0, $count=count($items); $i<$count; $i++){?>
      <tr>
       ...
       <?php } ?> 
       <?php if(in_array('image',$config_open)){ ?>
        <td align="center"><img src="<?=_upload_post.$items[$i]['thumb']?>" width="60" /></td>
      <?php } ?>
      <td class="title_name_data">
        <a href="index.html?com=post&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id_sub=<?=$items[$i]['id_sub']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" class="tipS SC_bold"><?=$items[$i]['name_vi']?></a>
      </td>
      <?php if(in_array('view',$config_open)){ ?> 
      ...
      <td class="actBtns">
        <a href="index.html?com=post&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id_sub=<?=$items[$i]['id_sub']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" title="" class="smallButton tipS" original-title="Sửa bài viết"><img src="Assets/images/icons/dark/pencil.png" alt=""></a>
        <?php if($type!='albumxxx'){?>
        <a href="index.html?com=post&act=delete&listid=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" onClick="if(!confirm('Xác nhận xóa')) return false;" title="" class="smallButton tipS" original-title="Xóa bài viết"><img src="Assets/images/icons/dark/close.png" alt=""></a>
        <?php } ?>
      </td>
    </tr>
  <?php } ?>
