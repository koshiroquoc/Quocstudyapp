<div class="support-online">
    <div class="support-content">
        <a href="tel:<?= preg_replace('/[^0-9]/','', $Setting['hotline'] )?>" title="call" target="_blank" class="call-now" rel="nofollow">
            <i class="fas fa-phone-volume"></i>
            <div class="animated infinite zoomIn kenit-alo-circle"></div>
            <div class="animated infinite pulse kenit-alo-circle-fill"></div>
            <span>Hotline: <?=preg_replace('/[^0-9]/','', $Setting['hotline'] )?></span>
        </a>
        <?php if($Setting['keymap']!=''): ?>
            <a class="mes" href="https://www.google.com/maps/dir/<?=$Setting['keymap']?>" title="Chỉ đường" target="_blank">
                <i class="fas fa-map-marked-alt"></i>
                <span>Chỉ đường</span>
            </a>
        <?php endif ?>

        <?php if($Setting['zalo']!=''): ?>
            <a class="zalo" href="http://zalo.me/<?=preg_replace('/[^0-9]/','', $Setting['zalo'] )?>" title="zalo" target="_blank">
                <img src="data:image/png;base64," alt="icon zalo">
                <span>Zalo: <?=preg_replace('/[^0-9]/','', $Setting['zalo'] )?></span>
            </a>
        <?php endif ?>

        <a class="sms" href="sms:<?= preg_replace('/[^0-9]/','', $Setting['hotline'] )?>" title="sms" target="_blank">
            <i class="fas fa-comments"></i>
            <span>SMS: <?=preg_replace('/[^0-9]/','', $Setting['hotline'] )?></span>
        </a>
    </div>
    <a class="btn-support">
        <div class="animated infinite zoomIn kenit-alo-circle"></div>
        <div class="animated infinite pulse kenit-alo-circle-fill"></div>
        <i class="fas fa-users text-primary"></i>
    </a>
</div>