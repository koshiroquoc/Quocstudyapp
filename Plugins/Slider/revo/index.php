			...
			<ul class="ul">
				<?php foreach ($Slider as $value): ?>
					<li data-transition="random-premium" <?=!empty($value['link']) ? 'data-link="'.$value['link'].'" data-target="_blank" data-slideindex="back"' : '' ?>>
						<img src="<?=_upload_hinhanh_l.'1366x520x1/'.$value['photo']?>" alt="<?=$value['name'] ?>" class="rev-slidebg" data-bgposition="center center" data-bgfit="cover" data-bgrepeat="no-repeat"/>
					</li>        
				<?php endforeach ?>
			</ul>
